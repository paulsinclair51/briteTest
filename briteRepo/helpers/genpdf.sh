...
# genpdf.sh - Internal PDF conversion helper used by gendocs.
#             Converts Markdown files to PDF, optionally removing or
#             converting <details> and <summary> tags.
#
...
Usage:
  genpdf.sh [<doc_options>] <document>[.md] [<outdocument>[.pdf]]
  genpdf.sh [<dir_options>] <indirectory> [<outdirectory>]
  genpdf.sh [<dir_options>]
  genpdf.sh [-b <backend>] -t
  genpdf.sh {-h | --help}

The third form is the same as: genpdf.sh [<dir_options>] ./
...
Examples:
  genpdf.sh docs/md/Guide.md
  genpdf.sh docs/md/Documentation_Guide
  genpdf.sh docs/md/Documentation_Guide.md docs/My_Documentation_Guide
  genpdf.sh docs/md
  genpdf.sh docs build
...
process_file() {
  ...
  # Extract the first PNG image line from the Markdown document and resolve it
  # relative to source_dir. The referenced document-specific PNG contains the
  # logo, brand name, and document title used by the generated document.
