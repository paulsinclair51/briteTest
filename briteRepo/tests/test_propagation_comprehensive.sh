...
echo "Test 3: Helper callable from any script in briteRepo/bin/"
echo "-------"
echo "Scenario: Call helper from a different script"
echo "Expected: Helper works the same regardless of caller"
echo ""

# Create a temporary test script in briteRepo/bin
test_script="$REPO_ROOT/briteRepo/bin/test_propagation_caller.$$"
...
echo "✓ Helper is callable from any script in briteRepo/bin/"
