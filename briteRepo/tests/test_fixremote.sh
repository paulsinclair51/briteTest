...
make_runner_repo() {
  local root="$1"
  mkdir -p "$root/briteRepo/bin" "$root/briteRepo/helpers" "$root/config" "$root/reports"

  cp "$FIXREMOTE_SRC" "$root/briteRepo/bin/fixremote"
  cp "$COMMON_HELPER_SRC" "$root/briteRepo/helpers/common.sh"
  cp "$GIT_HELPER_SRC" "$root/briteRepo/helpers/git_helpers.sh"
  cp "$REPORT_HELPER_SRC" "$root/briteRepo/helpers/report_helpers.sh"
  cp "$HEALTH_REPORT_HELPER_SRC" "$root/briteRepo/helpers/health_report.sh"
  cp "$CKROLE_HELPER_SRC" "$root/briteRepo/helpers/ckrole.sh"
  cp "$HEALTH_REPORT_HELPER_SRC" "$root/briteRepo/helpers/health_report.sh"
