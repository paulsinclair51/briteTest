...
assert_contains "**Changes:** " "$dry_report"
...
changes_line="$(grep -n '^\*\*Changes:\*\*' "$dry_report" | head -n 1 | cut -d: -f1)"
lines_line="$(grep -n '^\*\*Lines:\*\*' "$dry_report" | head -n 1 | cut -d: -f1)"
[[ -n "$commit_line" && -n "$command_line" && -n "$user_line" && -n "$changes_line" ]] || \
  fail "expected Commit/Command/User/Changes metadata lines"
...
[[ $((changes_line - user_line)) -eq 1 ]] || fail "expected Changes line immediately after User line"
[[ $((lines_line - changes_line)) -eq 1 ]] || fail "expected Lines line immediately after Changes line"
