...
CKSTYLE_SRC="$REPO_ROOT/briteRepo/helpers/ckstyle.sh"
...
make_fixture_repo() {
  ...
  mkdir -p "$repo/briteRepo/bin" "$repo/briteRepo/helpers" "$repo/config" \
    "$repo/docs/md" "$repo/docs/branding" "$repo/include" "$repo/src"

  cp "$REPORT_SRC" "$repo/briteRepo/bin/report"
  cp "$CKSTYLE_SRC" "$repo/briteRepo/helpers/ckstyle.sh"
