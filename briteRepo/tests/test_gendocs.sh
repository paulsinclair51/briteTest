...
briteRepo/helpers/genpdf.sh and briteRepo/helpers/gendocx.sh.
...
genpdf_helper="$repo_root/briteRepo/helpers/genpdf.sh"
gendocx_helper="$repo_root/briteRepo/helpers/gendocx.sh"
common_helper="$repo_root/briteRepo/helpers/common.sh"
git_helper="$repo_root/briteRepo/helpers/git_helpers.sh"
brand_name=""
...
EOF
  ...
  cat > "$repo/briteRepo/helpers/gendocx.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
  ...
  chmod +x "$repo/briteRepo/bin/gendocs" \
    "$repo/briteRepo/helpers/genpdf.sh" \
    "$repo/briteRepo/helpers/gendocx.sh"
  cp "$common_helper" "$repo/briteRepo/helpers/common.sh"
  cp "$git_helper" "$repo/briteRepo/helpers/git_helpers.sh"
  mkdir -p "$repo/config"
...
make_genpdf_helper_repo() {
  ...
  mkdir -p "$repo/briteRepo/helpers" "$repo/docs/branding" \
    "$repo/in/docs/branding" "$repo/in" "$repo/out" "$repo/bin"
  cp "$genpdf_helper" "$repo/briteRepo/helpers/genpdf.sh"
...
EOF
  ...
  chmod +x "$repo/briteRepo/helpers/genpdf.sh" "$repo/bin/pandoc"
  printf '%s\n' "$repo"
}
...
make_gendocx_helper_repo() {
  ...
  mkdir -p "$repo/briteRepo/helpers" "$repo/bin" "$repo/in" "$repo/out"
  cp "$gendocx_helper" "$repo/briteRepo/helpers/gendocx.sh"
...
EOF
  ...
  chmod +x "$repo/briteRepo/helpers/gendocx.sh" "$repo/bin/libreoffice"
  printf '%s\n' "$repo"
}
...
test_gendocs_missing_helper() {
  ...
  rm -f "$repo/briteRepo/helpers/gendocx.sh"
