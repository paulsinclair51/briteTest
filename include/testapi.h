...
/**
 * @name RA_TEST_VERSION
 *
 * @brief Version "M.m.p" for testapi.h which must be the same as
 *.       RA_TEST_VERSION_C for testapi.c.
 *
 ...
 * 
 * RA_VERSION_CMP(v1, v2)
 *    1 v1 is greater than v2,
 *    0 v1 is equal to v2,
 *   -1 v1 is less than v2,
 *.  -2 v1 or v2 has invalid version formatting,
 *
 *    v1 and v2 are strings with the same format as RA_RUNNER_VERSION,
 *    RA_RUNNER_VERFSION_C, RA_TEST_VERSION, and RA_TEST_VERISON_C.
 *
 * @{
 */

#if !defined(RA_RUNNER_VERSION)

size_t ta_internal_get_test_major_internal(const char *v);
#define RA_VERSION_MAJOR(v) ta_internal_get_test_major_internal((v))

size_t ta_internal_get_test_minor_internal(const char *v);
#define RA_VERSION_MINOR(v) ta_internal_get_test_minor_internal((v))
...
#define RA_VERSION_CMP(v1, v2) ta_internal_test_cmp_internal((v1), (v2))
...
typedef void *(*ra_malloc_fn)(size_t size);
typedef void *(*ra_realloc_fn)(void *ptr, size_t size);
typedef void (*ra_free_fn)(void *ptr);
 ...
 * pointer (for example, ta_read_file or ta_read_file_with_limit), the caller owns the
 * memory and must free it.
 *
 * @note Return conventions are grouped into three styles:
 * 1. Structured helpers return RA_OK, RA_MISMATCH, RA_TIMEOUT, or RA_E*.
...
/**
 * @name ta_assert_completes_within
 *
...
/**
 * @name ta_capture_standard_error
 *
...
/**
 * @name ta_with_working_directory
 *
 ...
 *
 * @return RA_OK on success, RA_ENOMEM on size exceeded, RA_EIO on read error.
 */
int ta_read_file(const char *path, char **content, size_t *length);

/**
 * @name ta_read_file_with_limit
 *
 ...
 *
 * @return RA_OK on success, RA_ESIZE if file exceeds max, RA_EIO on error.
 */
int ta_read_file_with_limit(const char *path,
                            size_t max_bytes,
                            char **content,
                            size_t *length);

/**
 * @name ta_read_file_into
 *
 ...
 *
 * @return RA_OK on success, RA_ESIZE if file exceeds buffer, RA_EIO on error.
 */
int ta_read_file_into(const char *path,
                      char *buffer,
                      size_t buffer_size,
                      size_t *out_length);
 ...
 * @return 0 on success, non-zero on error.
 */
int ta_stat_check(const char *path, int must_exist, long minimum_size);
...
/**
 * @name ta_compare_dirs
 *
 * @brief Compare two directories.
 *
 * @return RA_OK when the selected directory content matches, RA_MISMATCH when it differs,
 * or RA_EIO on directory traversal or stat failure.
 */
int ta_compare_dirs(const char *left_path, const char *right_path, int recursive);

/**
 * @name ta_compare_files
 *
...
enum {
  RA_FILECMP_IGNORE_TRAILING_WHITESPACE = 1 << 0,
  RA_FILECMP_IGNORE_EMPTY_LINES = 1 << 1,
  RA_FILECMP_IGNORE_TIMESTAMPS = 1 << 2,
  RA_FILECMP_IGNORE_LINE_ENDINGS = 1 << 3,
  RA_FILECMP_CASE_INSENSITIVE = 1 << 4
};
...
/**
 * @name ta_compare_paths_with_options
 *
 ...
 * @param options Optional comparison options. Pass NULL or RA_PATH_COMPARE_OPTIONS_INIT
 * for the default byte-for-byte comparison. options->flags uses RA_FILECMP_* values.
 *
 * @return RA_OK if equivalent, RA_MISMATCH if different, RA_EIO on error.
 */
...
/**
 * @name ta_compare_paths_ignoring_patterns
 *
 ...
 *
 * @return RA_OK if equivalent, RA_MISMATCH if different, RA_EPARSE/RA_EIO on error.
 */
...
/**
 * @name ta_compare_paths_masking_fields
 *
...
/**
 * @name ta_compare_paths_masking_ranges
 *
...
enum {
  RA_STATCMP_SIZE = 1 << 0,
  RA_STATCMP_PERMS = 1 << 1,
  RA_STATCMP_MTIME = 1 << 2,
  RA_STATCMP_OWNER = 1 << 3,
  RA_STATCMP_TYPE = 1 << 4
};
 ...
 * RA_PATH_METADATA_COMPARE_OPTIONS_INIT to compare size, permissions, and type.
 * options->flags uses RA_STATCMP_* values.
 *
 * @return RA_OK if selected metadata matches, RA_MISMATCH if different, RA_EIO on error.
 */
 ...
 *
 * @return RA_OK if equivalent, RA_MISMATCH if different, or RA_EPARSE on invalid JSON.
 */
...
/**
 * @name ta_compare_json_with_limit
 *
 ...
 * RA_JSON_COMPARE_OPTIONS_INIT to use the default size limit and strict key order.
 ...
 *
 * @return RA_OK if equivalent, RA_MISMATCH if different, RA_ESIZE/RA_EPARSE on error.
 */
...
/**
 * @name ta_compare_text_normalized
 *
 ...
 * RA_TEXT_COMPARE_OPTIONS_INIT for a literal text comparison.
 *
 * @return RA_OK when the texts match, RA_MISMATCH when they differ, or RA_EARG
 * on invalid input.
 */
...
/**
 * @name ta_with_environment_variable
 *
 ...
 * @param callback_context Callback context.
 *
 * @return Callback result when setup and restore succeeded, or RA_EARG/RA_EIO if
 ...
 *
 * @return RA_OK on success, RA_EIO on exec error.
 */
int ta_get_command_exit_code(const char *command_line, int *exit_code);

/**
 * @name ta_assert_command_exit_code
 *
 ...
 *
 * @return RA_OK if code matches, RA_MISMATCH if different, RA_EIO on error.
 */
 ...
 *
 * @return RA_OK on success, RA_EARG on invalid input, or RA_EIO on stat failure.
 */
 ...
 *
 * @return RA_OK if identical, RA_MISMATCH if different, RA_EIO on error.
 */
 ...
 *
 * @return RA_OK on success, RA_EIO on error.
 */
 ...
 *
 * @return RA_OK on success, RA_EARG on invalid input, RA_EPARSE if the path is not
 * found or cannot be parsed, or RA_ESIZE if extraction exceeds bounded storage.
 */
int ta_json_extract(const char *json_text, const char *path, char **out_value);

/**
 * @name ta_json_has_path
 *
 ...
 *
 * @return RA_OK on success, RA_ENOMEM on limit reached.
 */
int ta_cleanup_register(void (*cleanup_callback)(void *cleanup_context),
                        void *cleanup_context);

/**
 * @name ta_temp_file_auto
 *
