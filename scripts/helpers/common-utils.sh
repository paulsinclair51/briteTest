...
# Args: \$1 = file path
# Example: assert_file_exists \"scripts/helpers/ckbranchname.sh\"
assert_file_exists() {
  local file=\"\$1\"
  if [[ ! -f \"\$file\" ]]; then
...
git_log_range() {
  local base=\"\$1\"
  local head=\"\$2\"
  git log \"origin/\${base}..origin/\${head}\" --format=%H 2>/dev/null || echo \"\"
}
...
git_changed_files() {
  local base=\"\$1\"
  local head=\"\$2\"
  git diff \"origin/\${base}...origin/\${head}\" --name-only 2>/dev/null || echo \"\"
}
