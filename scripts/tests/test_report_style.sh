...
CKSTYLE_SRC="$REPO_ROOT/scripts/helpers/ckstyle.sh"
...
make_fixture_repo() {
  ...
  mkdir -p "$repo/scripts/bin" "$repo/scripts/helpers" "$repo/config" \
    "$repo/docs/md" "$repo/docs/branding" "$repo/include" "$repo/src"

  cp "$REPORT_SRC" "$repo/scripts/bin/report"
  cp "$CKSTYLE_SRC" "$repo/scripts/helpers/ckstyle.sh"
...
rc=$(run_capture "$TMPDIR/default.out" bash -lc "cd '$WORK' && bash ./scripts/bin/report style")
...
rc=$(run_capture "$TMPDIR/verbose.out" bash -lc "cd '$WORK' && bash ./scripts/bin/report style -v -f docs/md/Release_v1.0.0.md -f include/runnerapi.h -f src/runnerapi.c")
...
rc=$(run_capture "$TMPDIR/long-verbose.out" bash -lc "cd '$WORK' && bash ./scripts/bin/report style --verbose")
...
rc=$(run_capture "$TMPDIR/basename.out" bash -lc "cd '$WORK' && bash ./scripts/bin/report style -f Release_v1.0.0.md")
...
reset_report_dir "$WORK"
rc=$(run_capture "$TMPDIR/header-basename.out" bash -lc "cd '$WORK' && bash ./scripts/bin/report style -f runnerapi.h")
