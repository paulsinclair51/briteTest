...
# 4) A clone-path run should check only the specified clone target
...
assert_contains "## Target: Specified Clone Repository" "$clone_report"
if grep -Fq -- "## Target: This Repository" "$clone_report"; then
  fail "clone-path run should not include this repository as a second target"
fi
