...
# test_feedback.sh - smoke tests for scripts/bin/feedback.
#
...
FEEDBACK_SRC="$REPO_ROOT/scripts/bin/feedback"
...
cp "$FEEDBACK_SRC" "$WORK/scripts/bin/feedback"
...
chmod +x "$WORK/scripts/bin/feedback"
...
rc=$(run_capture "$TMPDIR/help.out" open bash -c "cd '$WORK' && bash ./scripts/bin/feedback -h")
...
rc=$(run_capture "$TMPDIR/view-browser.out" open env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./scripts/bin/feedback view -b")
...
rc=$(run_capture "$TMPDIR/browser-only-open.out" open env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./scripts/bin/feedback -b")
[[ "$rc" -eq 0 ]] || fail "feedback -b should exit 0 when an open PR exists (got $rc)"
assert_contains "pr view 7 --web" "$TMPDIR/gh.log"
pass "browser-only mode opens open PR"

rc=$(run_capture "$TMPDIR/view-browser-closed.out" closed env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./scripts/bin/feedback view -b")
...
rc=$(run_capture "$TMPDIR/browser-only-closed.out" closed env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./scripts/bin/feedback -b")
...
rc=$(run_capture "$TMPDIR/view-no-pr.out" none env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./scripts/bin/feedback view -b")
...
rc=$(run_capture "$TMPDIR/browser-only-no-pr.out" none env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./scripts/bin/feedback -b")
...
rc=$(run_capture "$TMPDIR/update-unsupported.out" open env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./scripts/bin/feedback update -c 'nope'")
...
rc=$(run_capture "$TMPDIR/sync-unsupported.out" open env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./scripts/bin/feedback sync")
...
rc=$(run_capture "$TMPDIR/respond.out" open env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./scripts/bin/feedback respond -i 102 -c 'Thanks for the clarification.'")
...
rc=$(run_capture "$TMPDIR/resolve-denied.out" open env GITHUB_ACTOR=testcontrib bash -c "cd '$WORK' && bash ./scripts/bin/feedback resolve -i 102")
...
rc=$(run_capture "$TMPDIR/resolve.out" open env GITHUB_ACTOR=testapprover bash -c "cd '$WORK' && bash ./scripts/bin/feedback resolve -i 102")
...
rc=$(run_capture "$TMPDIR/disapprove.out" open env GITHUB_ACTOR=testapprover bash -c "cd '$WORK' && bash ./scripts/bin/feedback disapprove -- 'Please add tests.'")
