...
rc=$(run_capture "$TMPDIR/unreachable.out" bash "$UNREACHABLE_REMOTE_REPO/scripts/bin/fixlocal" -d -t 1)
...
rc=$(run_capture "$TMPDIR/tracking-refresh-fail.out" env PATH="$TRACKING_FAKEBIN:$PATH" bash "$TRACKING_FAIL_REPO/scripts/bin/fixlocal" -t 5)
...
rc=$(run_capture "$TMPDIR/timeout-classification.out" env PATH="$TIMEOUT_FAKEBIN:$PATH" bash "$TIMEOUT_REPO/scripts/bin/fixlocal" -d -t 1)
