...
[[ "$rc" -eq 4 ]] || fail "fixlocal -d -r 1 with unreachable origin should exit 4 (got $rc)"
...
rc=$(run_capture "$TMPDIR/tracking-refresh-fail.out" env PATH="$TRACKING_FAKEBIN:$PATH" bash "$TRACKING_FAIL_REPO/scripts/bin/fixlocal" -r 5)
...
rc=$(run_capture "$TMPDIR/timeout-classification.out" env PATH="$TIMEOUT_FAKEBIN:$PATH" bash "$TIMEOUT_REPO/scripts/bin/fixlocal" -d -r 1)
