...
rc=$(run_capture "$TMPDIR/skip-e-policy.out" \
  bash -lc "cd '$WORK' && bash ./scripts/bin/pull -e")
...
rc=$(run_capture "$TMPDIR/not-worktree.out" \
  bash -lc "cd '$TMPDIR' && bash '$WORK/scripts/bin/pull'")
...
rc=$(run_capture "$TMPDIR/detached.out" bash -lc "cd '$WORK' && bash ./scripts/bin/pull")
...
rc=$(run_capture "$TMPDIR/main-policy.out" bash -lc "cd '$WORK' && bash ./scripts/bin/pull")
...
rc=$(run_capture "$TMPDIR/main-owner-policy.out" env \
  ...
  bash -lc "cd '$WORK' && bash ./scripts/bin/pull -o")
...
rc=$(run_capture "$TMPDIR/remote-timeout.out" bash -lc \
  "cd '$WORK' && PATH='$TMPDIR/fake-bin':\$PATH bash ./scripts/bin/pull -t 1")
...
pull_note="$(git -C "$WORK" notes --ref=briteTest-workflow show HEAD)"
