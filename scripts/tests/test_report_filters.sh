...
rc=$(run_capture "$TMPDIR/multiple-records.out" bash -lc \
	"cd '$WORK' && bash ./scripts/bin/report branch -q 'Record-Group: appended pair' -l 0")
...
rc=$(run_capture "$TMPDIR/delimiter-detail.out" bash -lc \
	"cd '$WORK' && bash ./scripts/bin/report branch -q '--- briteTest workflow --- marker'")
...
assert_contains "recorded pull details with --- briteTest workflow --- marker" \
	"$WORK/$delimiter_rel"
...
rc=$(run_capture "$TMPDIR/malformed-quiet.out" bash -lc \
	"cd '$WORK' && bash ./scripts/bin/report branch -q 'retarget activity'")
...
rc=$(run_capture "$TMPDIR/malformed-verbose.out" bash -lc \
	"cd '$WORK' && bash ./scripts/bin/report branch -v -q 'retarget activity'")
...
rc=$(run_capture "$TMPDIR/snapshot-current-remote.out" bash -lc \
	"cd '$WORK' && bash ./scripts/bin/report branch -r -t 2 -q 'remote state after snapshot'")
[[ "$rc" -eq 0 ]] || \
...
tracked_report="$WORK/reports/local-20000101-000002.md"
printf 'tracked local report\n' > "$tracked_report"
git -C "$WORK" add reports/local-20000101-000002.md
...
rc=$(run_capture "$TMPDIR/subdirectory.out" bash -lc \
	"cd '$WORK/nested/report-test' && bash ../../scripts/bin/report branch")
...
set +e
bash -c "cd '$WORK' && bash ./scripts/bin/report branch -l 2" \
...
concurrent_report_count="$(find "$WORK/reports" -maxdepth 1 -type f \
	-name 'local-*.md' ! -name 'local-20000101-000002.md' | \
	wc -l | tr -d ' ')"
...
concurrent_report="$(find "$WORK/reports" -maxdepth 1 -type f \
	-name 'local-*.md' ! -name 'local-20000101-000002.md' -print -quit)"
