...
rc=$(run_capture "$TMPDIR/nonzero-patch.out" env GITHUB_ACTOR=testuser bash -lc "cd '$WORK' && bash ./scripts/bin/rmbranch -r 'v1.0.1'")
...
rc=$(run_capture "$TMPDIR/version-protected.out" env GITHUB_ACTOR=testuser bash -lc "cd '$WORK' && bash ./scripts/bin/rmbranch -r 'v1.0.0'")
