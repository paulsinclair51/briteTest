...
PUSHUP_SRC="$REPO_ROOT/scripts/helpers/pushup_parent.sh"
...
cp "$PUSHUP_SRC" "$WORK/scripts/helpers/pushup_parent.sh"
cp "$PUSH_SRC" "$WORK/scripts/bin/push"
chmod +x "$WORK/scripts/helpers/pushup_parent.sh" "$WORK/scripts/bin/push"
...
rc=$(run_capture "$TMPDIR/skip-e.out" env PATH="$FAKEBIN:$PATH" GITHUB_ACTOR=testowner bash -lc "cd '$WORK' && bash ./scripts/helpers/pushup_parent.sh -e")
...
run_pushup() {
  ...
  (
    ...
    env "${envvars[@]}" bash ./scripts/helpers/pushup_parent.sh "${pushup_args[@]}"
  ) >"$outfile" 2>&1
  local rc=$?
  set -e
  echo "$rc"
