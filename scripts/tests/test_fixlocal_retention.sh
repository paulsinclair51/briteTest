...
rc=$(run_capture "$TMPDIR/retention-dry.out" bash "$RETENTION_DRY_REPO/scripts/bin/fixlocal" -d -t 5)
...
rc=$(run_capture "$TMPDIR/retention-nondry.out" bash "$RETENTION_NON_DRY_REPO/scripts/bin/fixlocal" -t 5)
