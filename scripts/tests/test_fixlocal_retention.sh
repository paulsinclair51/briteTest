...
# shellcheck source=fixlocal_test_lib.sh
source "$SCRIPT_DIR/fixlocal_test_lib.sh"
...
old_dry_report="$RETENTION_DRY_REPO/reports/repository/repository-d-20000101-000000-111.md"
old_non_dry_report="$RETENTION_DRY_REPO/reports/repository/repository-20000101-000000-222.md"
echo "old dry" > "$old_dry_report"
echo "old non-dry" > "$old_non_dry_report"
touch -d '40 days ago' "$old_dry_report" "$old_non_dry_report"

rc=$(run_capture "$TMPDIR/retention-dry.out" bash "$RETENTION_DRY_REPO/scripts/bin/fixlocal" -d -r 5)
...
old_dry_report_nd="$RETENTION_NON_DRY_REPO/reports/repository/repository-d-20000101-000000-311.md"
old_non_dry_report_nd="$RETENTION_NON_DRY_REPO/reports/repository/repository-20000101-000000-322.md"
recent_non_dry_report_nd="$RETENTION_NON_DRY_REPO/reports/repository/repository-20990101-000000-333.md"
...
rc=$(run_capture "$TMPDIR/retention-nondry.out" bash "$RETENTION_NON_DRY_REPO/scripts/bin/fixlocal" -r 5)
