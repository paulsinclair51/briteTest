...
case $result in
  ...
  2) echo "✗ FAIL: Return code 2 (should have succeeded)" ;;
esac
echo ""
...
echo "Call 1:"
time bt_propagate_repository_history >/dev/null 2>&1 && echo "Result: 0 (up to date)"
echo ""
...
test_script="/tmp/test_caller.sh"
...
if [[ -f "$REPO_ROOT/logs/repository_history.md" ]]; then
  cat "$REPO_ROOT/logs/repository_history.md" | head -20
  echo ""
