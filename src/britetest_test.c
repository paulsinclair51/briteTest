/**
 * @file /paulsinclair51/src/britetest_test.c
 *
...
#define BT_ALLOC_MAX_BYTES ((size_t)(16u * 1024u * 1024u))

/* Global allocator hooks. */
static bt_malloc_fn g_malloc_fn = malloc;
static bt_realloc_fn g_realloc_fn = realloc;
static bt_free_fn g_free_fn = free;
...
static int bt_add_overflow_size(size_t a, size_t b, size_t *out)
{
  if (!out) {
    return BT_EARG;
  }
...
static int bt_mul_overflow_size(size_t a, size_t b, size_t *out)
{
  if (!out) {
    return BT_EARG;
  }
...
static void *bt_malloc_checked(size_t n, int *err)
{
  if (err) *err = 0;
  if (n == 0 || n > BT_ALLOC_MAX_BYTES) {
    if (err) *err = BT_ESIZE;
    return NULL;
  }
  void *p = g_malloc_fn(n);
  if (!p && err) *err = BT_ENOMEM;
  return p;
}

static void *bt_realloc_checked(void *p, size_t n, int *err)
{
  if (err) *err = 0;
  if (n == 0 || n > BT_ALLOC_MAX_BYTES) {
    if (err) *err = BT_ESIZE;
  ...
  if (!next && err) *err = BT_ENOMEM;
  return next;
}

static char *bt_strdup_checked(const char *s, int *err)
{
  size_t n;
  char *dup;
  ...
  n = strnlen(s, BT_ALLOC_MAX_BYTES + 1);
  if (n > BT_ALLOC_MAX_BYTES) {
    if (err) *err = BT_ESIZE;
    return NULL;
  }

  int alloc_err = 0;
  dup = (char *)bt_malloc_checked(n + 1, &alloc_err);
  if (!dup) {
...
static long long bt_now_ms(void)
{
  struct timespec ts;
                  ...
                  int poll_interval_ms)
{
  ...
  for (;;) {
    ...
    if (bt_now_ms() - start >= timeout_ms) {
      return 0;
    }

    bt_sleep_ms(poll_interval_ms);
  }
}
                               ...
                               int timeout_ms)
{
  ...
  for (;;) {
    ...
    if (timeout_ms > 0 && (bt_now_ms() - start_ms) >= timeout_ms) {
      kill(pid, SIGKILL);
...
static int bt_set_nonblock(int fd)
{
               ...
               int *exit_code)
{
  ...
  for (;;) {
    ...
    if (timeout_ms > 0) {
      long long elapsed = bt_now_ms() - start_ms;
...
int bt_read_file_with_limit(const char *path, size_t max_bytes, char **content, size_t *length)
{
  FILE *f;
  char *buf;
  size_t cap = 1024;
  size_t limit = BT_ALLOC_MAX_BYTES;
  size_t used = 0;
  int alloc_err = 0;
  ...
  *length = 0;
  ...
  if (max_bytes > 0) {
    if (max_bytes > BT_ALLOC_MAX_BYTES) {
      fclose(f);
      return BT_ESIZE;
    }
    limit = max_bytes;
  ...
  buf = (char *)bt_malloc_checked(cap + 1, &alloc_err);
  if (!buf) {
    fclose(f);
    return alloc_err;
  }
  ...
  for (;;) {
    ...
    if (used == cap) {
      ...
      next = (char *)bt_realloc_checked(buf, next_cap + 1, &alloc_err);
      if (!next) {
        g_free_fn(buf);
        fclose(f);
        return alloc_err;
...
{
  return bt_read_file_with_limit(path, 0, content, length);
}

int bt_read_file_into(const char *path, char *buffer, size_t buffer_size, size_t *out_length)
{
  FILE *f;
  size_t n;
...
static int bt_remove_tree_impl(const char *path)
{
  struct stat st;
  ...
  if (S_ISDIR(st.st_mode)) {
    ...
    while ((ent = readdir(dir)) != NULL) {
      ...
      if (bt_remove_tree_impl(child) != 0) {
        closedir(dir);
        return -1;
      }
    }
...
int bt_remove_tree(const char *path)
{
  ...
  return bt_remove_tree_impl(path);
}

int bt_stat_check(const char *path, int must_exist, long min_size)
{
  struct stat st;
...
static int bt_cmp_file_result(int rc)
{
  if (rc > 0) {
    return BT_OK;
  }
...
static size_t bt_line_content_len(const char *line, size_t len, int ignore_line_endings)
{
  if (len > 0 && line[len - 1] == '\n') {
    --len;
...
static size_t bt_trimmed_line_len(const char *line, size_t len, int ignore_line_endings)
{
  len = bt_line_content_len(line, len, ignore_line_endings);
...
static int bt_line_is_empty_for_compare(const char *line, size_t len, int flags)
{
  size_t i;

  len = bt_line_content_len(line, len, (flags & BT_FILECMP_IGNORE_LINE_ENDINGS) != 0);
  for (i = 0; i < len; ++i) {
...
static size_t bt_span_timestamp_token(const char *s, size_t len)
{
  size_t i = 0;
...
static int bt_append_compare_char(char **dst, size_t *remaining, char c)
{
...
static int bt_normalize_compare_line(const char *line,
                                     size_t len,
                                     int flags,
                                     char **out_line)
{
  ...
  size_t i;
  int alloc_err = 0;
  int ignore_line_endings = (flags & BT_FILECMP_IGNORE_LINE_ENDINGS) != 0;

  if (!line || !out_line) {
    return BT_EARG;
  }

  content_len = (flags & BT_FILECMP_IGNORE_TRAILING_WHITESPACE) != 0
                  ? bt_trimmed_line_len(line, len, ignore_line_endings)
                  : bt_line_content_len(line, len, ignore_line_endings);
  out_cap = (content_len * 2) + 8;
  buf = (char *)bt_malloc_checked(out_cap, &alloc_err);
  if (!buf) {
    return alloc_err;
  }
  ...
  for (i = 0; i < content_len; ++i) {
    ...
    if ((flags & BT_FILECMP_IGNORE_TIMESTAMPS) != 0) {
      span = bt_span_timestamp_token(line + i, content_len - i);
      if (span > 0) {
        if (bt_append_compare_char(&dst, &remaining, '<') != BT_OK ||
            bt_append_compare_char(&dst, &remaining, 'T') != BT_OK ||
            bt_append_compare_char(&dst, &remaining, 'S') != BT_OK ||
            bt_append_compare_char(&dst, &remaining, '>') != BT_OK) {
          g_free_fn(buf);
          return BT_ESIZE;
        }
        i += span - 1;
    ...
    if ((flags & BT_FILECMP_CASE_INSENSITIVE) != 0) {
      c = (char)tolower((unsigned char)c);
    }
    if (bt_append_compare_char(&dst, &remaining, c) != BT_OK) {
      g_free_fn(buf);
      return BT_ESIZE;
    }
  }
  ...
  if (!ignore_line_endings && len > content_len) {
    if (len >= 2 && line[len - 2] == '\r' && line[len - 1] == '\n') {
      if (bt_append_compare_char(&dst, &remaining, '\r') != BT_OK ||
          bt_append_compare_char(&dst, &remaining, '\n') != BT_OK) {
        g_free_fn(buf);
        return BT_ESIZE;
      }
    } else if (line[len - 1] == '\n') {
      if (bt_append_compare_char(&dst, &remaining, '\n') != BT_OK) {
        g_free_fn(buf);
        return BT_ESIZE;
      }
    }
  ...
  if (bt_append_compare_char(&dst, &remaining, '\0') != BT_OK) {
    g_free_fn(buf);
    return BT_ESIZE;
  }
...
static int bt_compile_patterns(const char **patterns, size_t pattern_count, regex_t **out_patterns)
{
  regex_t *compiled;
  size_t i;
  int alloc_err = 0;
  ...
  *out_patterns = NULL;
  ...
  compiled = (regex_t *)bt_malloc_checked(sizeof(*compiled) * pattern_count, &alloc_err);
  if (!compiled) {
    return alloc_err;
  }
...
static void bt_free_patterns(regex_t *patterns, size_t pattern_count)
{
  size_t i;
...
static int bt_line_matches_patterns(const char *line,
                                    size_t len,
                                    regex_t *patterns,
                                    size_t pattern_count)
{
  ...
  content_len = bt_line_content_len(line, len, 1);
  copy = (char *)bt_malloc_checked(content_len + 1, &alloc_err);
  if (!copy) {
    return 0;
  }
...
static int bt_next_compare_line(FILE *f,
                                char **line,
                                size_t *cap,
                                ssize_t *len,
                                int flags,
                                ...
                                size_t pattern_count)
{
  ...
  for (;;) {
    ...
    if ((flags & BT_FILECMP_IGNORE_EMPTY_LINES) != 0 &&
        bt_line_is_empty_for_compare(*line, (size_t)read_len, flags)) {
      continue;
    }
    if (bt_line_matches_patterns(*line, (size_t)read_len, patterns, pattern_count)) {
      continue;
    }
...
static int bt_compare_file_streams(FILE *f1,
                                   FILE *f2,
                                   int flags,
                                   regex_t *patterns,
                                   size_t pattern_count)
{
  ...
  for (;;) {
    ...
    rc = bt_next_compare_line(f1, &line1, &cap1, &len1, flags, patterns, pattern_count);
    if (rc != BT_OK) {
      break;
    }
    rc = bt_next_compare_line(f2, &line2, &cap2, &len2, flags, patterns, pattern_count);
    if (rc != BT_OK) {
      break;
    }

    if (len1 < 0 || len2 < 0) {
      rc = (len1 == len2) ? BT_OK : BT_MISMATCH;
      break;
    }

    rc = bt_normalize_compare_line(line1, (size_t)len1, flags, &norm1);
    if (rc != BT_OK) {
      g_free_fn(norm1);
      break;
    }
    rc = bt_normalize_compare_line(line2, (size_t)len2, flags, &norm2);
    if (rc != BT_OK) {
      g_free_fn(norm1);
      g_free_fn(norm2);
      break;
...
static int bt_field_name_matches(const char *name,
                                 size_t name_len,
...
static char *bt_skip_ws_chars(char *p)
{
...
static char *bt_mask_json_value(char *p)
{
  char open;
  char close;
  int depth;
...
static void bt_mask_field_values(char *text, const char **field_names, size_t field_count)
{
  char *p;
  int line_start = 1;
  ...
  while (*p) {
    if (line_start) {
      ...
      name_end = line;
      cursor = bt_skip_ws_chars(line);
      if ((*cursor == '=' || *cursor == ':') &&
          bt_field_name_matches(name_start, (size_t)(name_end - name_start), field_names, field_count)) {
        cursor = bt_skip_ws_chars(cursor + 1);
        while (*cursor && *cursor != '\n' && *cursor != '\r') {
    ...
    if (*p == '"') {
      ...
      if (*q == '"') {
        char *after = bt_skip_ws_chars(q + 1);
        if (*after == ':' && bt_field_name_matches(key_start, (size_t)(q - key_start), field_names, field_count)) {
          p = bt_mask_json_value(after + 1);
        }
      }
    }
...
int bt_compare_file_to_path(FILE *file, const char *path)
{
  ...
  rc = bt_compare_files(file, other);
  fclose(other);
  return rc;
}
...
{
  return bt_compare_file_to_path(file, path);
}
                                  ...
                                  const bt_path_compare_options_t *options)
{
  ...
  if (flags == 0) {
    return bt_cmp_file_result(bt_compare_paths(left_path, right_path));
  }
  ...
  rc = bt_compare_file_streams(f1, f2, flags, NULL, 0);
  fclose(f1);
  fclose(f2);
  return rc;
}
                               ...
                               size_t pattern_count)
{
  ...
  rc = bt_compile_patterns(ignore_patterns, pattern_count, &patterns);
  if (rc != BT_OK) {
    return rc;
  }
  ...
  f1 = fopen(left_path, "rb");
  if (!f1) {
    bt_free_patterns(patterns, pattern_count);
    return BT_EIO;
  }
  f2 = fopen(right_path, "rb");
  if (!f2) {
  ...
  rc = bt_compare_file_streams(f1, f2, 0, patterns, pattern_count);
  fclose(f1);
  fclose(f2);
  bt_free_patterns(patterns, pattern_count);
  return rc;
                           ...
                           size_t field_count)
{
  ...
  rc = bt_read_file_with_limit(left_path, 0, &text1, &len1);
  if (rc != BT_OK) {
    return rc;
  }
  rc = bt_read_file_with_limit(right_path, 0, &text2, &len2);
  if (rc != BT_OK) {
    g_free_fn(text1);
    return rc;
  }
  ...
  (void)len1;
  (void)len2;
  bt_mask_field_values(text1, field_names, field_count);
  bt_mask_field_values(text2, field_names, field_count);

  rc = (strcmp(text1, text2) == 0) ? BT_OK : BT_MISMATCH;
  g_free_fn(text1);
  g_free_fn(text2);
  return rc;
}

static int bt_offset_is_masked(size_t offset,
                           ...
                           size_t range_count)
{
  ...
  rc = bt_read_file_with_limit(left_path, 0, &buf1, &len1);
  if (rc != BT_OK) {
    return rc;
  }
  rc = bt_read_file_with_limit(right_path, 0, &buf2, &len2);
  if (rc != BT_OK) {
    g_free_fn(buf1);
    return rc;
  }
  ...
  for (i = 0; i < limit; ++i) {
    ...
    if (bt_offset_is_masked(i, skip_offsets, skip_lengths, range_count)) {
      continue;
    }
    if (!in1 || !in2) {
      rc = BT_MISMATCH;
...
static int bt_mode_type(mode_t mode)
{
                             ...
                             const bt_path_metadata_compare_options_t *options)
{
  ...
  if (flags == 0) {
    flags = BT_STATCMP_SIZE | BT_STATCMP_PERMS | BT_STATCMP_TYPE;
  }
  ...
  if ((flags & BT_STATCMP_SIZE) != 0 && st1.st_size != st2.st_size) {
    return BT_MISMATCH;
  }
  if ((flags & BT_STATCMP_PERMS) != 0 &&
      (st1.st_mode & 07777) != (st2.st_mode & 07777)) {
    return BT_MISMATCH;
  }
  if ((flags & BT_STATCMP_MTIME) != 0 && st1.st_mtime != st2.st_mtime) {
    return BT_MISMATCH;
  }
  if ((flags & BT_STATCMP_OWNER) != 0 &&
      (st1.st_uid != st2.st_uid || st1.st_gid != st2.st_gid)) {
    return BT_MISMATCH;
  }
  if ((flags & BT_STATCMP_TYPE) != 0 &&
      bt_mode_type(st1.st_mode) != bt_mode_type(st2.st_mode)) {
    return BT_MISMATCH;
  }
...
static int bt_cmp_entry_type(mode_t m1, mode_t m2)
{
...
static int bt_dircmp_impl(const char *dir1, const char *dir2, int recursive)
{
  DIR *d1;
  DIR *d2;
  struct dirent *ent;
  ...
  while ((ent = readdir(d1)) != NULL) {
    ...
    if (!bt_cmp_entry_type(s1.st_mode, s2.st_mode)) {
      closedir(d1);
      closedir(d2);
      return BT_MISMATCH;
    }
    ...
    if (S_ISREG(s1.st_mode)) {
      int frc = bt_compare_paths(p1, p2);
      if (frc != 1) {
        closedir(d1);
        closedir(d2);
        return (frc < 0) ? BT_EIO : BT_MISMATCH;
      }
    } else if (S_ISDIR(s1.st_mode) && recursive) {
      int dir_rc = bt_dircmp_impl(p1, p2, recursive);
      if (dir_rc != BT_OK) {
        closedir(d1);
        closedir(d2);
        return dir_rc;
  ...
  while ((ent = readdir(d2)) != NULL) {
    ...
    if (access(p1, F_OK) != 0) {
      closedir(d1);
      closedir(d2);
      return (errno == ENOENT) ? BT_MISMATCH : BT_EIO;
    }
  }
...
int bt_compare_dirs(const char *left_path, const char *right_path, int recursive)
{
  ...
  return bt_dircmp_impl(left_path, right_path, recursive != 0);
}
...
static int bt_sb_reserve(bt_sb_t *sb, size_t add)
{
  size_t need;
  size_t new_cap;
  char *next;
  ...
  if (bt_add_overflow_size(sb->len, add, &need) != 0 ||
      bt_add_overflow_size(need, 1, &need) != 0) {
    return BT_ESIZE;
  }
  if (need <= sb->cap) {
    return BT_OK;
  ...
  while (new_cap < need) {
    ...
    new_cap *= 2;
    if (new_cap > BT_ALLOC_MAX_BYTES) {
      new_cap = BT_ALLOC_MAX_BYTES;
      break;
    }
  }

  next = (char *)bt_realloc_checked(sb->data, new_cap, &err);
  if (!next) {
    return err;
  }
...
static int bt_sb_append_n(bt_sb_t *sb, const char *s, size_t n)
{
  if (!sb || (!s && n > 0) || bt_sb_reserve(sb, n) != 0) {
    return BT_EARG;
  }
  if (n > 0) {
...
static int bt_sb_append_c(bt_sb_t *sb, char c)
{
  return bt_sb_append_n(sb, &c, 1);
}

static void bt_json_skip_ws(const char **p)
{
...
static int bt_json_append_string(const char **p, bt_sb_t *out)
{
  const char *start;
  int escape = 0;
  ...
  while (**p) {
    ...
    if (c == '"') {
      return bt_sb_append_n(out, start, (size_t)(*p - start)) ? BT_ESIZE : BT_OK;
    }
  }
...
static int bt_json_append_number(const char **p, bt_sb_t *out)
{
  const char *start = *p;
...
static int bt_json_append_literal(const char **p, bt_sb_t *out)
{
  if (strncmp(*p, "true", 4) == 0) {
    *p += 4;
    return bt_sb_append_n(out, "true", 4) ? BT_ESIZE : BT_OK;
  }
  if (strncmp(*p, "false", 5) == 0) {
    *p += 5;
    return bt_sb_append_n(out, "false", 5) ? BT_ESIZE : BT_OK;
  }
  if (strncmp(*p, "null", 4) == 0) {
    *p += 4;
    return bt_sb_append_n(out, "null", 4) ? BT_ESIZE : BT_OK;
  }
  return BT_EPARSE;
}

static int bt_json_normalize_value(const char **p, bt_sb_t *out, int sort_keys);

static int bt_json_kv_cmp(const void *a, const void *b)
{
  const bt_json_kv_t *ka = (const bt_json_kv_t *)a;
  const bt_json_kv_t *kb = (const bt_json_kv_t *)b;
  return strcmp(ka->key, kb->key);
}

static int bt_json_normalize_object(const char **p, bt_sb_t *out, int sort_keys)
{
  bt_json_kv_t *items = NULL;
  size_t count = 0;
  size_t cap = 0;
  int rc = BT_EPARSE;
  size_t i;

  if (**p != '{' || bt_sb_append_c(out, '{') != 0) {
    return BT_EPARSE;
  }
  ++(*p);
  bt_json_skip_ws(p);
  ...
  if (**p == '}') {
    ++(*p);
    return bt_sb_append_c(out, '}') ? BT_ESIZE : BT_OK;
  }
  ...
  while (**p) {
    ...
    bt_json_skip_ws(p);
    if (bt_json_append_string(p, &key) != 0) {
      g_free_fn(key.data);
      goto cleanup;
    }
    ...
    bt_json_skip_ws(p);
    if (bt_json_normalize_value(p, &value, sort_keys) != 0) {
      g_free_fn(key.data);
    ...
    if (count == cap) {
      ...
      if (next_cap < cap ||
          bt_mul_overflow_size(next_cap, sizeof(*items), &bytes) != 0 ||
          bytes > BT_ALLOC_MAX_BYTES) {
        g_free_fn(key.data);
        g_free_fn(value.data);
        goto cleanup;
      }

      next = (bt_json_kv_t *)bt_realloc_checked(items, bytes, &alloc_err);
      if (!next) {
        g_free_fn(key.data);
        g_free_fn(value.data);
        goto cleanup;
  ...
  if (sort_keys && count > 1) {
    qsort(items, count, sizeof(*items), bt_json_kv_cmp);
  }

  for (i = 0; i < count; ++i) {
    if (i > 0 && bt_sb_append_c(out, ',') != 0) {
      goto cleanup;
    }
    if (bt_sb_append_n(out, items[i].key, strlen(items[i].key)) != 0 ||
        bt_sb_append_c(out, ':') != 0 ||
        bt_sb_append_n(out, items[i].value, strlen(items[i].value)) != 0) {
      goto cleanup;
    }
  }
  if (bt_sb_append_c(out, '}') != 0) {
    goto cleanup;
  }
...
static int bt_json_normalize_array(const char **p, bt_sb_t *out, int sort_keys)
{
  int first = 1;
  ...
  while (**p) {
    bt_json_skip_ws(p);
    if (!first && bt_sb_append_c(out, ',') != 0) {
      return BT_ESIZE;
    }
    if (bt_json_normalize_value(p, out, sort_keys) != 0) {
      return BT_EPARSE;
    }
    first = 0;
...
static int bt_json_normalize_value(const char **p, bt_sb_t *out, int sort_keys)
{
  ...
  if (**p == '{') {
    return bt_json_normalize_object(p, out, sort_keys);
  }
  if (**p == '[') {
    return bt_json_normalize_array(p, out, sort_keys);
  }
  if (**p == '"') {
    return bt_json_append_string(p, out);
  }
  if (**p == 't' || **p == 'f' || **p == 'n') {
    return bt_json_append_literal(p, out);
  }
  return bt_json_append_number(p, out);
}

static char *bt_json_normalize(const char *s, int sort_keys)
{
  const char *p = s;
  bt_sb_t out = {0};
  ...
  if (!out.data && bt_sb_append_c(&out, '\0') != 0) {
    return NULL;
  }
                               ...
                               const bt_json_compare_options_t *options)
{
  ...
  if (max_bytes > 0 && max_bytes < BT_ALLOC_MAX_BYTES) {
  ...
  e = bt_json_normalize(expected_json, ignore_key_order != 0);
  a = bt_json_normalize(actual_json, ignore_key_order != 0);
  if (!e || !a) {
    if (e) g_free_fn(e);
    if (a) g_free_fn(a);
    return BT_EPARSE;
  ...
  rc = strcmp(e, a) == 0 ? BT_OK : BT_MISMATCH;
  g_free_fn(e);
  g_free_fn(a);
  return rc;
}
...
{
  return bt_compare_json_with_limit(expected_json, actual_json, NULL);
}

static int bt_match_impl(const char *text, const char *pattern)
{
  while (*pattern) {
    if (*pattern == '*') {
      while (*pattern == '*') {
...
int bt_match(const char *text, const char *pattern)
{
  ...
  return bt_match_impl(text, pattern) ? 1 : 0;
}
                                 ...
                                 void *callback_context)
{
  ...
  if (had_existing) {
    int err = 0;
    old_value = bt_strdup_checked(existing, &err);
    if (!old_value) {
      return BT_ENOMEM;
    }
  }
...
int bt_assert_command_exit_code(const char *command_line, int expected_code)
{
  ...
  rc = bt_get_command_exit_code(command_line, &actual_code);
  if (rc != BT_OK) {
    return rc;
  }

  return (actual_code == expected_code) ? BT_OK : BT_MISMATCH;
}
...
int bt_compare_file_lines(const char *left_path, const char *right_path)
{
  ...
  fclose(f1);
  fclose(f2);
  return match ? BT_OK : BT_MISMATCH;
}
...
int bt_rename_file(const char *old_path, const char *new_path)
{
  ...
  return (rename(old_path, new_path) == 0) ? BT_OK : BT_EIO;
}
...
int bt_symlink(const char *target, const char *link_path)
{
  ...
  return (symlink(target, link_path) == 0) ? BT_OK : BT_EIO;
}

/* JSON data extraction test helper functions. */

static int bt_json_extract_simple(const char *json_text, const char *path, char **out_value)
{
    ...
    if (*path_p == '[') {
      ...
      if (key_len > 0) {
        ...
        if (*p == '"') {
          ...
          if (bt_sb_append_n(&out, val_start, p - val_start) != 0) {
            g_free_fn(out.data);
            return BT_ESIZE;
          }
          if (*p == '"') {
...
{
  return bt_json_extract_simple(json_text, path, out_value);
}
...
int bt_json_has_path(const char *json_text, const char *path)
{
  ...
  rc = bt_json_extract(json_text, path, &value);
  if (value) {
    g_free_fn(value);
  }
...
#define BT_MAX_CLEANUPS 64
typedef struct {
  void (*fn)(void *);
  void *ctx;
} bt_cleanup_entry_t;

static bt_cleanup_entry_t g_cleanups[BT_MAX_CLEANUPS];
static size_t g_cleanup_count = 0;

int bt_cleanup_register(void (*cleanup_callback)(void *cleanup_context),
                        void *cleanup_context)
{
  if (!cleanup_callback || g_cleanup_count >= BT_MAX_CLEANUPS) {
    return BT_ENOMEM;
  }
...
static void bt_temp_file_cleanup(void *ctx)
{
  char *path = (char *)ctx;
  if (path) {
    unlink(path);
...
int bt_temp_file_auto(const char *suffix, char *outpath, size_t outpathsz)
{
  ...
  rc = bt_cleanup_register(bt_temp_file_cleanup, template_str);
  if (rc != BT_OK) {
    unlink(template_str);
    g_free_fn(template_str);
    return rc;
